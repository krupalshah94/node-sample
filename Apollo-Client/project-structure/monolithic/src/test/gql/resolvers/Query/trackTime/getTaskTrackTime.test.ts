...
type TQuery = { getTaskTrackTime: WorkLog };
...
const GET_TASK_TRACK_TIME_QUERY = `query ($taskId: ID!) {
  getTaskTrackTime(taskId: $taskId) {
      id
      time
      date
...
describe('gql/resolvers/Query/trackTime/getTaskTrackTime', () => {
  ...
  // success
  it('should allow to get task track time', async () => {
    ...
    const result = data?.getTaskTrackTime;
  ...
  it('should allow to get empty task track time list', async () => {
