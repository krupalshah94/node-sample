...
type TQuery = { getTrackTime: TrackTime };
...
const GET_TRACK_TIME_QUERY = `query ($contractId: ID!) {
  getTrackTime(contractId: $contractId) {
      id
      startTime
      endTime
...
describe('gql/resolvers/Query/trackTime/getTrackTime', () => {
  ...
  it('should allow to get track time', async () => {
    ...
    const result = data?.getTrackTime;
  ...
  it('should allow to get empty track time list', async () => {
  ...
  it("other user have't access to contract", async () => {
    ...
    const { errors } = await execQuery<TQuery>(
      GET_TRACK_TIME_QUERY,
      {
        contractId: _.get(outputData, ['trackTime', 'id'])
      },
      otherUser
    );
