...
type TQuery = { retryFailedPayouts: Payment[] };
...
describe('gql/resolvers/Mutation/retryFailedPayout', () => {
  it.skip('is same "gql/resolvers/Mutation/retryFailedPayouts"', () => {});
});

describe('gql/resolvers/Mutation/retryFailedPayouts', () => {
  ...
  it('should allow to retry failed charges', async () => {
    ...
    const result = data?.retryFailedPayouts;
  ...
  describe('', () => {
      ...
      await getClient(async client => {
        await async.each(payments!, async payment => {
          await PaymentOperationModel.update.exec(
            ...
            {
              id: _.get(payment, ['payout', 'id']),
              status: _.get(payment, ['payout', 'status'])
            },
            ctx
          );
        });
    ...
    it('cannot retry payouts that are not failed', async () => {
      ...
      Test.Check.error(errors, new GraphQLError(`You cannot retry payouts that were not failed`));
    });
  });

  it('only full pro owner have access', async () => {
