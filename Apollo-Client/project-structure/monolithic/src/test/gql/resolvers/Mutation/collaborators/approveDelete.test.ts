...
type TQuery = { approveDeleteCollaborator: DeletedRecord };
...
const APPROVE_DELETE_COLLABORATOR_MUTATION = `mutation ($collaboratorId: ID!) {
  approveDeleteCollaborator(collaboratorId: $collaboratorId) {
    id
    deleted
  }
}`;

describe('gql/resolvers/Mutation/collaborators/approveDelete', () => {
  ...
  // error
  it(`cannot approve delete collaborator with different role`, async () => {
    ...
    const { errors } = await execQuery<TQuery>(
      APPROVE_DELETE_COLLABORATOR_MUTATION,
      {
    ...
    Test.Check.error(
      errors,
      new GraphQLError(
        `You cannot approve to delete collaborator with another role because you are ${proUser.role!.name}`,
        403
      )
    );
  });
  ...
  // success
  it('should allow to approve delete collaborator', async () => {
    const contract = outputData.contract;
    ...
    const result = data?.approveDeleteCollaborator;
