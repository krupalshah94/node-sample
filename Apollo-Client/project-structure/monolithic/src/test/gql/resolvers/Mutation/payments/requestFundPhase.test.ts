...
type TQuery = { requestFundPhase: Phase };
...
const enum ContractName {
  RequestFundPhase = 'RequestFundPhase'
}
...
const REQUEST_FUND_PHASE_MUTATION = `mutation ($phaseId: ID!, $autoPayoutRequest: Boolean) {
  requestFundPhase(phaseId: $phaseId, autoPayoutRequest: $autoPayoutRequest) {
    id
    name
    description
    divisionTrade
...
describe('gql/resolvers/Mutation/payments/requestFundPhase', () => {
  ...
  const inputData = {
    ...
    contract: {
      name: ContractName.RequestFundPhase
    },
    phases: [
      {
  ...
  it('should allow to request fund phase', async () => {
    ...
    const result = data?.requestFundPhase;
  ...
  it(`Fund phase already requested`, async () => {
    ...
    Test.Check.error(errors, new GraphQLError(`Fund phase already requested.`));
  });
