...
type TQuery = { addMessageTag: Message };
...
const ADD_TAG_TO_MESSAGE_MUTATION = `mutation ($messageId: ID!, $tag: String!) {
  addMessageTag(messageId: $messageId, tag: $tag) {
    id
    chatId
    fromId
    text
...
describe('gql/resolvers/Mutation/chats/messages/tags/add', () => {
  ...
  describe('', () => {
    ...
    after(async () => {
      ...
      await getClient(async client => {
        await MessageModel.removeTag.exec(
          client,
          {
            messageId: (_.get(messageInDirect, 'id') as unknown) as string,
            tag
          },
          ctx
        );
    ...
    it('should allow to add tag to a message', async () => {
      const { data, errors } = await execQuery<TQuery>(
        ADD_TAG_TO_MESSAGE_MUTATION,
        {
      ...
      const result = data?.addMessageTag;
  ...
  describe('', () => {
    ...
    it('should allow collaborator to add tag to a message in a direct chat', async () => {
      const { data, errors } = await execQuery<TQuery>(
        ADD_TAG_TO_MESSAGE_MUTATION,
        {
          messageId: _.get(messageInDirect, 'id'),
  ...
  describe('', () => {
    ...
    it('adding an existing tag should cause an error', async () => {
      ...
      Test.Check.error(errors, new GraphQLError('Message not updated.'));
    });
  });
  ...
  it(`collaborator hasn't write access in a group chat`, async () => {
    ...
    const messageInGroup = _.find(outputData.messages, ['chatId', groupChat.id]);
