...
import { DeletedRecord } from '../../../../../gql/resolvers/DeletedRecord';
/*other*/
import { Test } from '../../../../helpers/Test';

type TQuery = { deleteCollaborator: Collaborator | DeletedRecord };
...
const DELETE_COLLABORATOR_MUTATION = `mutation ($collaboratorId: ID!) {
  deleteCollaborator(collaboratorId: $collaboratorId) {
    ... on DeletedRecord {
      id
      deleted
    }
...
describe('gql/resolvers/Mutation/collaborators/delete', () => {
  ...
  // error
  it(`collaborator already requested to delete`, async () => {
    ...
    Test.Check.error(errors, new GraphQLError(`Already requested to delete`));
  });
  ...
  // success
  it('should allow to request delete collaborator', async () => {
    const contract = outputData.contract;
    ...
    const result = data?.deleteCollaborator;
    ...
    Test.Check.data(
      result as Collaborator,
      {
        ...
        invitedById: collaboratorFullHome.invitedById,
        contractId: contract.id,
        createdAt: {
          $check: '==',
          $value: new Date(),
  ...
  it('should allow to delete collaborator', async () => {
    const contract = outputData.contract;
    ...
    Test.Check.data(result as DeletedRecord, {
