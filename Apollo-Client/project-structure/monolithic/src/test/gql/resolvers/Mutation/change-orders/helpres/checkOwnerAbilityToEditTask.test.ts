...
import { checkOwnerAbilityToEditTask } from '../../../../../../gql/resolvers/Mutation/change-orders/helpers/checkOwnerAbilityToEditTask';
/*other*/
...
describe('gql/resolvers/Mutation/change-order/helpers/checkOwnerAbilityToEditTask', () => {
  ...
  // success
  it('should allow assigned owner to edit task', async () => {
    ...
    try {
      await getClient(async client => {
        ...
        await checkOwnerAbilityToEditTask(client, { tasks: data }, ctx as any);
      });
    } catch (e) {
      error = e;
    } finally {
  ...
  it('should allow to edit task if is not owner', async () => {
  ...
  // error
  it(`forbidden non-associated owner to edit the task`, async () => {
    const homeUser = _.find(outputData.users, { email: Email.Home });
    if (!homeUser) throw GraphQLError.notFound('home user');
    ...
    try {
      ...
      Test.Check.error(e, new GraphQLError(`You cannot update task if you are not assigned.`));
    }
  });
});
