...
type TQuery = { deleteScheduleWorkTime: Schedule };
...
const enum ScheduleName {
  TestName = PhaseName.Schedule + 'TestName'
}
...
const DELETE_SCHEDULE_WORK_TIME_MUTATION = `mutation (
  $scheduleId: ID!
) {
  deleteScheduleWorkTime(scheduleId: $scheduleId) {
      id
      estimatedTime
...
describe('gql/resolvers/Mutation/schedule/delete', () => {
  ...
  before(async () => {
    ...
    outputData = await getClientTransaction(async client => {
      ...
      const collaboratorReadPro = _.find(users, {
        email: Email.Collaborator + CollaboratorPermission.Read + UserRole.Pro
      });
      if (!collaboratorReadPro) {
      ...
      const phases = await Promise.all(
        _.map(inputData.project.contracts, async ({ phase: phaseData, name: contractName }) => {
          ...
          await Promise.all(
            _.map(phaseData.tasks, async taskData => {
      ...
      const schedules = await Promise.all(
        _.map(schedulePhase.tasks, async task => {
          ...
          await scheduleGenerate.create({
            roleId: collaboratorReadPro.lastRoleId,
  ...
  it("other user haven't access to contract", async () => {
    ...
    const { errors } = await execQuery<TQuery>(
      DELETE_SCHEDULE_WORK_TIME_MUTATION,
      {
  ...
  it('should allow to update schedule', async () => {
    ...
    const scheduleData = _.get(inputData, 'schedule');
    ...
    const result = data?.deleteScheduleWorkTime;
    ...
    Test.Check.data(
      result,
      {
        ...
        'worker.id': _.get(schedule, 'roleId')
      },
      requiredFieldSet
    );
  });
