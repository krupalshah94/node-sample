...
type TQuery = { stopTrackTime: TrackTime };
...
const STOP_TRACK_TIME_MUTATION = `mutation ($trackTimeId: ID!) {
  stopTrackTime(trackTimeId: $trackTimeId) {
      id
      startTime
      endTime
...
describe('gql/resolvers/Mutation/trackTime/stopTrackTime', () => {
  ...
  it('should allow to stop track time', async () => {
    ...
    const result = data?.stopTrackTime;
