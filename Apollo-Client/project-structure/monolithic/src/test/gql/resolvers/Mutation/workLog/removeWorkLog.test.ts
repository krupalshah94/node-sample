...
type TQuery = { removeWorkLog: WorkLog };
...
const REMOVE_WORK_LOG_MUTATION = `mutation ($workLogId: ID!) {
  removeWorkLog(workLogId: $workLogId) {
      id
      time
      date
...
describe('gql/resolvers/Mutation/workLog/removeWorkLog', () => {
  ...
  // success
  it('should allow to remove work log', async () => {
    ...
    const result = data?.removeWorkLog;
